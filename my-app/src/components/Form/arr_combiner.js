    ...
    if (listArray.length !== 0) {
       ...
       for (let i = 0; i < listArray.length; i++) {
            ...
            if (listArray[i].date === date) {
                const previouslyCompletedDistance = parseFloat(listArray[i].distance);
                updatedItemArray.distance = parseFloat(distance) + previouslyCompletedDistance; // прибавляем ранее пройденную дистанцию к новому значению
                listArray.splice(i, 1, updatedItemArray); // заменяем текущий элемент на обновлённый
                hasReplaced = true;
            };
        
            if ((hasReplaced === false) && (!isNaN(updatedItemArray))) listArray.push(updatedItemArray);    // если замены элемента не было, то добавляем новый элемент в конец списка
        };
